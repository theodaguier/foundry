...
pub fn write_foundry_directory(
    ...
    let foundry_dir = project_dir.join(".foundry");
    let contracts_dir = foundry_dir.join("contracts");
    let skills_dir = foundry_dir.join("skills");
    let review_dir = foundry_dir.join("review");

    fs::create_dir_all(&contracts_dir).map_err(|e| e.to_string())?;
    ...
    fs::write(
        foundry_dir.join("brief.md"),
        format!(
            "# {}\n\nType: {}\nDescription: {}\n",
            plugin_name, plugin_role, user_prompt
        ),
    )
    .map_err(|e| e.to_string())?;

    fs::write(
        contracts_dir.join("plan.md"),
        "# Implementation Plan\n\n(To be generated by Planner phase)\n",
    )
    .map_err(|e| e.to_string())?;

    fs::write(contracts_dir.join("parameter-manifest.json"), "[]").map_err(|e| e.to_string())?;

    fs::write(
        review_dir.join("findings.md"),
        "# Code Review Findings\n\n(To be generated by Review phase)\n",
    )
    .map_err(|e| e.to_string())?;

    let main_skills = crate::models::agent::skills_for_plugin_type(plugin_type);
    for skill in &main_skills {
        let skill_path = skills_dir.join(format!("{}.md", skill.as_str()));
        let content = match skill {
            crate::models::agent::SkillId::JuceExpert => {
                include_str!("../../../foundry-kit/skills/juce-expert/SKILL.md")
            }
            crate::models::agent::SkillId::ArtDirector => {
                include_str!("../../../foundry-kit/skills/art-director/SKILL.md")
            }
            crate::models::agent::SkillId::SoundEngineer => {
                include_str!("../../../foundry-kit/skills/sound-engineer/SKILL.md")
            }
            crate::models::agent::SkillId::Beatmaker => {
                include_str!("../../../foundry-kit/skills/beatmaker/SKILL.md")
            }
        };
...
## Requirements
- Write Source/PluginProcessor.h and Source/PluginProcessor.cpp with real DSP
- Write Source/PluginEditor.h, Source/PluginEditor.cpp, and Source/FoundryLookAndFeel.h with real UI
- Use APVTS for all parameters with smooth updates
- Factory presets using getNumPrograms/getProgramName
- Do NOT touch CMakeLists.txt
"#,
        name = name,
        role = plugin_role,
