...
function classifyFailure(message: string) {
  ...
  if (
    ...
    return {
      ...
      subtitle:
        "Foundry stopped before build because JUCE or the local build toolchain is not configured correctly.",
      kind: "environment",
    };
  }

  if (
    lower.includes("did not create the required source files") ||
    lower.includes("missing source")
  ) {
    return {
      title: "Incomplete Generation",
      subtitle:
        "The generator stopped before writing all required JUCE source files.",
      kind: "generation",
    };
  }
  ...
  if (lower.includes("timed out") || lower.includes("watchdog")) {
    return {
      ...
      subtitle: "The code generator did not finish within the allowed time.",
      kind: "generation",
    };
  }
  ...
  if (lower.includes("no plugin bundles found") || lower.includes("bundle")) {
    return {
      title: "Build Output Invalid",
      subtitle:
        "The build completed without producing a usable AU or VST3 bundle.",
      kind: "build",
    };
  }
  ...
  if (
    ...
    return {
      title: "Build Failed",
      subtitle: "Foundry could not compile the plugin into a working build.",
      kind: "build",
    };
  }
  ...
  if (
    ...
    lower.includes("administrator privileges") ||
    lower.includes("permission")
  ) {
    return {
      title: "Installation Failed",
      subtitle:
        "The plugin was generated, but Foundry could not install it on your system.",
      kind: "install",
    };
  }

  if (
    lower.includes("cli is not available") ||
    ...
    return {
      ...
      subtitle:
        "Foundry could not start the code generation agent in the current environment.",
      kind: "agent",
    };
  }
  ...
  return {
    ...
    subtitle: "Foundry could not finish a usable plugin from this brief.",
    kind: "unknown",
  };
}
...
export default function ErrorPage() {
  ...
  const message =
    (location.state as any)?.message || "An unknown error occurred";
  const { title: failureTitle, subtitle: failureSubtitle, kind } =
    ...
    <div className="flex flex-col items-center justify-center h-full gap-5 px-6">
      <svg
        ...
        style={{
          ...
          transform: iconAppeared ? "scale(1)" : "scale(0.92)",
        }}
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
      ...
      <div
        ...
        style={{
          ...
          transform: textAppeared ? "translateY(0)" : "translateY(4px)",
        }}
      >
        ...
        <div className="w-full max-w-[720px] border border-border bg-muted/60 rounded-md overflow-hidden mt-3">
          <div className="px-3 py-2 border-b border-border text-[10px] tracking-[1px] text-muted-foreground/70">
            RAW ERROR
          </div>
          <div className="px-3 py-2 font-mono text-xs text-foreground whitespace-pre-wrap break-words">
            {message}
          </div>
        </div>

        {recentLogs.length > 0 && (
          <div className="w-full max-w-[720px] border border-border bg-muted/40 rounded-md overflow-hidden">
            <div className="px-3 py-2 border-b border-border text-[10px] tracking-[1px] text-muted-foreground/70">
              LAST LOG LINES
            </div>
            <div className="px-3 py-2 font-mono text-xs text-muted-foreground space-y-1">
      ...
      <div
        ...
        style={{
          ...
          transform: actionsAppeared ? "translateY(0)" : "translateY(6px)",
        }}
      >
