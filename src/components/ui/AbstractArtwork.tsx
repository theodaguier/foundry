 ...
 */
export default function AbstractArtwork({ pluginType, className = "" }: Props) {
  return (
...
/** 8 vertical bars at varying heights — Swift: WaveformBarsArtwork */
...
/** 3 concentric circles — Swift: ConcentricRingsArtwork */
...
/** 5 spectrum bars with baseline — Swift: SpectrumBarsArtwork */
function SpectrumBars() {
  const heights = [0.4, 0.7, 1.0, 0.6, 0.3];
  return (
    <svg viewBox="0 0 50 40" className="w-[50px] h-[40px] opacity-20">
      {/* Baseline */}
      <line x1={0} y1={39} x2={50} y2={39} stroke="currentColor" strokeWidth={1} className="text-[var(--color-text-primary)]" />
      {heights.map((h, i) => (
        <rect
          key={i}
          x={i * 10 + 1}
